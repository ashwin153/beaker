...
-> consensus
   ...
   -> remove myself as participant
   -> 

Reconfigure(Configuration of me and seed)
 -> Proposes an empty transaction on seed and my configuration.
 -> Returns the current configuration.
 -> Propose the new configuration with me as a spectator/participant on the old configuration.

If a proposal is prepared with an older configuration, then reply with the current configuration.
Otherwise, reply normally.

If the proposer receives a newer configuration when it prepares, it updates its own configuration
and retries.

Do not accept proposals with older configurations. 
If a proposal is accepted with a newer configuration, then update my configuration.

Learn proposals when a quorum of their configuration vote for it.

If a reconfigure is learned, a majority have the new configuration and any proposals with the old
configuration should be learned.

Proposals are ordered by configuration and then by ballot.


Prepare (P)
  If C < P(C), then our configuration is stale.
  If C = P(C), then our configurations are identical.
  If C > P(C), then their configuration is stale.
...
Accept (P, Q(P))
  if Q <= Q(P), then vote for P

Learn (P, Q(P))
  if P has Q(P) votes, then commit
